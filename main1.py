...
class Optimizer_Adam:
    ...
    def update_params(self, layer):
        ...
        if hasattr(layer, 'kernels'):
            ...
            layer.kernel_momentums = self.beta_1 * \
                                     layer.kernel_momentums + \
                                     (1 - self.beta_1) * layer.dkernels
            layer.bias_momentums = self.beta_1 * \
                                   layer.bias_momentums + \
                                   (1 - self.beta_1) * layer.dbiases
            kernel_momentums_corrected = layer.kernel_momentums / \
                                         (1 - self.beta_1 ** (self.iterations + 1))
            bias_momentums_corrected = layer.bias_momentums / \
                                       (1 - self.beta_1 ** (self.iterations + 1))
            layer.kernel_cache = self.beta_2 * layer.kernel_cache + \
                                 (1 - self.beta_2) * layer.dkernels ** 2
            layer.bias_cache = self.beta_2 * layer.bias_cache + \
                               (1 - self.beta_2) * layer.dbiases ** 2
            kernel_cache_corrected = layer.kernel_cache / \
                                     (1 - self.beta_2 ** (self.iterations + 1))
            bias_cache_corrected = layer.bias_cache / \
                                   (1 - self.beta_2 ** (self.iterations + 1))

            layer.kernels += -self.current_learning_rate * \
                             kernel_momentums_corrected / \
                             (np.sqrt(kernel_cache_corrected) +
                              self.epsilon)
        ...
        for layer in self.trainable_layers:
            if hasattr(layer, 'kernels'):
                if hasattr(layer, 'kernel_regularizer'):
                    ...
                    layer.bias_regularizer_l2 = 0

                if layer.kernel_regularizer_l1 > 0:
...
def load_mnist_dataset(dataset, path, items):
# Scan all the directories and create a list of labels
    ...
    return np.array(X)[:items], np.array(y).astype('uint8')[:items]

def create_data_mnist(path, classes, items):
    # Load both sets separately
    X, y = load_mnist_dataset('train', path, items)
    X_test, y_test = load_mnist_dataset('test', path, items)
    y = to_categorical(y)
    y = y.reshape(len(y), classes)
    # And return all the data
    return X, y, X_test, y_test

X, y, X_test, y_test = create_data_mnist('fashion_mnist_images', classes, items)
X = (X.reshape(X.shape[0], -1).astype(np.float32) - 127.5) / 127.5
X_test = (X_test.reshape(X_test.shape[0], -1).astype(np.float32) -
             127.5) / 127.5
X = X.reshape((items, 1, 28, 28))
X_test = X_test.reshape((items, 1, 28, 28))
...
model.add(Convolutional((items, 1, 28, 28), 3, 5))
model.add(Activation_Sigmoid())
model.add(Reshape((items, 5, 26, 26), (items, 5 * 26 * 26, 1)))
...
model.train(X, y, validation_data=(X_test, y_test), epochs=15, batch_size=items, print_every=100)
print("-" * 21)
